...
// 互斥信号量编号：保护环形缓冲的元数据与槽位
const SEM_MUTEX: usize = 0;
// 空槽计数信号量编号：表示可写入的剩余槽位数
const SEM_EMPTY: usize = 1;
// 已有元素计数信号量编号：表示可读取的元素数
...
/// 生产者线程：向环形缓冲写入自身标识(id)，总共写入 NUMBER_PER_PRODUCER 次
unsafe fn producer(id: *const usize) -> ! {
    ...
    for _ in 0..NUMBER_PER_PRODUCER {
        ...
        // 获取互斥：独占访问 FRONT(写指针) 与 BUFFER(缓冲区)
        semaphore_down(SEM_MUTEX);
        // 写入数据：此处用生产者标识(id) 作为简单负载
        BUFFER[FRONT] = id;
        // 推进写指针：保持环形不变量 FRONT < BUFFER_SIZE(容量)
